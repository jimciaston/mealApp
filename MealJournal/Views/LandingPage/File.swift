...
//  MealJournal
//
//  Created by Jim Ciaston on 5/17/22.
//
...
func grabAges() {
    ...
    let filteredAge = try! NSRegularExpression(pattern: "age=(\\d+)") //searches age==, then followed by one to two digits
    let ages = filteredAge.matches(in: data, range: NSRange(data.startIndex..., in: data))
        .compactMap { match -> Int? in //filters ages to Int's
            guard let range = Range(match.range(at: 1), in: data),
            let age = Int(data[range]), age > 50 else { return nil }
            return age
        }
    print(ages.count)
}
