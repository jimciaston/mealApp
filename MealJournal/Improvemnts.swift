//
//  Improvemnts.swift
//  MealJournal
//
//  Created by Jim Ciaston on 11/11/22.
//

import Foundation
///
///1) Add delete to custom food items
///

/// 2) able to delete recipes
///
///3) MealJournal  - add tap entry and pop up window
///4) if you go to navigation view of journal, does not dismiss if you select a meal
///5) dont let user loop if going back in caeldnar. example if its mnday only let them go back to Sunday, not monday so it halts confusion
///6) remove arrow if on "Today"
